...
const OlympicsTrendingNews = () => {
  ...
  const [combinedOlympicsNews, setCombinedOlympicsNews] =
    useState([]);
      ...
      try {
        ...
        if (country) {
          responseArray = await Promise.all([
            axios.get(`${BASE_URL}/news/olympics?page=${page}`),
          ]);
        } else {
          setCombinedOlympicsNews([]);
          setIsLoading(false);
          return;
        }
        ...
        setCombinedOlympicsNews(combinedNews);
      } catch (error) {
      ...
      <div className="news__card-cont">
        {isLoading ? (
          ...
          <>
            ...
            {combinedOlympicsNews.length > visibleCards && (
              <button
