...
const HealthNews = () => {
  ...
  const [combinedHealthNews, setCombinedHealthNews] = useState([]);
    ...
    try {
      ...
      if (country) {
        responseArray = await Promise.all([
          axios.get(`${BASE_URL}/news/theconversation?page=${page}`),
          axios.get(`${BASE_URL}/news/statnews?page=${page}`),
          axios.get(`${BASE_URL}/news/kffhealth?page=${page}`),
          axios.get(`${BASE_URL}/news/medpagetoday?page=${page}`),
        ]);
      } else {
        ...
        {isLoading ? (
          ...
          <>
           {renderNewsArticles(
            ...
            {combinedHealthNews.length > visibleCards && (
              <button
