...
const HealthNews = () => {
  ...
  const [combinedBusinessNews, setCombinedHealthNews] = useState([]);
    ...
    try {
      ...
      if (country) {
        responseArray = await Promise.all([
          axios.get(`${BASE_URL}/news/theconversation`),
          axios.get(`${BASE_URL}/news/statnews`),
          axios.get(`${BASE_URL}/news/kffhealth`),
          axios.get(`${BASE_URL}/news/medpagetoday`),
        ]);
      } else {
        setCombinedHealthNews([]);
        setIsLoading(false);
        return;
      }
      ...
      setCombinedHealthNews(combinedNews);
    } catch (error) {
    ...
    <>
      <div className="news__card-cont">
        {isLoading ? (
          ...
          <>
            {renderNewsArticles(combinedBusinessNews)}
            {combinedBusinessNews.length > visibleCards && (
