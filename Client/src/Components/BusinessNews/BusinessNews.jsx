...
const BusinessNews = () => {
  ...
  const fetchData = useCallback(async () => {
    try {
      setIsLoading(true);
      if (!country) {
        setCombinedBusinessNews([]);
        setIsLoading(false);
        return;
      }

      const responseArray = await Promise.all([
        axios.get(`${BASE_URL}/news/marketbusiness`),
        axios.get(`${BASE_URL}/news/ibtimes`),
        axios.get(`${BASE_URL}/news/vox`),
        axios.get(`${BASE_URL}/news/fortune`),
        axios.get(`${BASE_URL}/news/businessinsider`),
      ]);
      ...
      setCombinedBusinessNews(combinedNews);
    } catch (error) {
